 ...
 *
 * [已修正 const 错误]
 * [已修正 frame_id 问题 - 现在会递增 frame_id]
 */
void BYTETracker::resync_kalman_filters(const vector<STrack>& klt_tracks)
{
    // CRITICAL FIX: Increment frame_id since we're processing a new frame
    // Without this, intermediate frames have the same frame_id as the last keyframe,
    // causing update() to reject the new data because frame_id hasn't advanced
    this->frame_id++;
    int current_frame_id = this->frame_id;
